...
import { deriveDynamicStates, deriveDynamicCategories } from "../utils/derivedData";
...
export default function HomePage({
  ...
  const dynamicStates = deriveDynamicStates(jobsData);
  const dynamicCats = deriveDynamicCategories(jobsData);
        ...
        <div className="hero__inner">
          <div>
            <div className="hero__live-pill">
              ...
              {jobsData.length}+ Active Job Notifications Today
            </div>
            ...
            <div className="search-bar search-bar--page hero__search">
              ...
              <select value={state} onChange={(e) => setState(e.target.value)} className="search-bar__select">
                {["All States", ...dynamicStates.map((s) => s.name)].map((sItem) => (
      ...
      <div className="page-layout">
        <div>
          ...
          <SectionHeader
            ...
            sub={`${jobsData.length} vacancies found`}
            action="View All"
            onAction={() => {
              setFilter({ q: "", state: "", cat: "", edu: "All" });
              setPage("jobs");
            }}
          />
          ...
          >
            Load All {jobsData.length} Jobs →
          </button>
        </div>

        <Sidebar setPage={setPage} latestJobs={jobsData} onViewJob={onViewJob} />
      </div>
    </div>
  );
}
